...
- [ ] Decide whether to include `gh-issues` dependency-contract cleanup (`gh` requirement vs curl-only claim) in the same skills pass
...
- Skill quality drift detected (`canvas` missing frontmatter + stale `clawd` path docs; `model-usage` has unresolved Linux TODO; `ordercli` marked partial/WIP for Deliveroo support)
